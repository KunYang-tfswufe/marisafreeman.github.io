...
matches:
  ...
  - trigger: "`q"
    replace: |-
      # 1. 角色与任务 (策略1：编写清晰的说明)
      - **扮演角色**: 你现在是一名【请填写具体角色，例如：资深市场分析师、经验丰富的程序员、富有创意的文案写手】。
      - **核心任务**: 你的主要任务是【请清晰、简洁地描述你要AI完成的核心目标，例如：分析一份财报、编写一个Python脚本、创作一则广告语】。
      
      # 2. 背景与上下文 (策略1 & 2：清晰说明 & 提供参考)
      - **背景信息**: 【提供任务相关的背景，例如：本次分析的目标受众是潜在投资者，他们对技术细节不感兴趣，更关注市场潜力和盈利能力】。
      - **参考资料/文本**: 
        """
        【在此处粘贴你需要AI处理的原始文本、数据、代码示例或其他参考材料。这对于需要总结、翻译、分析或模仿风格的任务至关重要。】
        """
      
      # 3. 工作流程与步骤 (策略3 & 4：拆分任务 & 给予思考时间)
      请严格按照以下步骤完成任务：
      1.  **第一步 (分析/思考)**: 首先，请对上述【参考资料】进行深入分析，并【列出你的关键发现、思考过程、或一个初步大纲】。请在这一步不要生成最终答案。
      2.  **第二步 (草拟/执行)**: 基于第一步的分析，开始【执行核心任务，例如：撰写草稿、编写代码、生成方案】。
      3.  **第三步 (审查/优化)**: 对第二步生成的内容进行自我审查和优化，确保它满足所有的要求。
      
      # 4. 输出要求 (策略1 & 6：清晰说明 & 系统测试)
      - **输出格式**: 请以【请指定明确的格式，例如：Markdown格式的表格、JSON对象、要点列表、不超过200字】的形式输出最终结果。
      - **风格与语气**: 整体风格要求【专业、严谨、风趣、口语化等】。
      - **关键约束**: 
          - 【必须包含关键词 A、B、C】
          - 【内容不能涉及...】
          - 【数据必须引用自提供的参考资料】
      ...
      # 5. 外部知识/工具 (策略5：使用外部工具)
      - **(可选)** 如果你需要执行计算或获取实时信息，请【明确说明，例如：假设你可以使用代码解释器来计算增长率，或者假设你可以访问最新的网络信息来验证事实】。
      ---
```
